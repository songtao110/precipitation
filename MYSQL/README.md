...
如果排序的列是无序的，那么排序的结果行的顺序也是不确定的。

一般情况下，组合效率更高更快(相比于单列索引) 
...
3.若排序内容不能够全部放入内存，则分批次将排序好的内容放入文件，然后将多个文件进行归并排序
4.若排序中包含limit语句，则使用对堆排序优化排序过程
5.order by语句使用索引最左前列，使用where子句与order by子句条件列组合满足索引最左前列

MySql的高版本采用order by limit 进行排序时，如果排序字段没有索引，优化器采用不稳定排序，会导致数据缺失或重复。一定要在排序字段创建索引。如果是多字段，一定要创建联合索引。索引字段值且不能为NULL，确保使用索引组织排序。去或者是采用主键字段作为排序
```
